    ...
    public void insertOrUpdate(K key, V value, BplusTree<K, V> tree) {
        ...
        if (isLeaf) {
            ...
            if (parent != null) {
                ...
                parent.entries_val.add(index, right.entries_val.get(0));
                ...
                parent.entries_val.add(right.entries_val.get(0));
                entries = null;
                entries_val = null;
                children = null;
            }
        ...
        for (int i = 0; i < entries.size(); i++) {
            if (leftSize != 0) {
                ...
                if (!b && entries.get(i).getKey().compareTo(key) > 0) {
                    ...
                    left.entries_val.add(entries_val.get(i));
                }
            } else {
                if (!b && entries.get(i).getKey().compareTo(key) > 0) {
                    Set<V> vals = new HashSet<>();
                    ...
                    right.entries_val.add(entries_val.get(i));
                }
            }
        }
        if (!b) {
    ...
    protected void updateInsert(BplusTree<K, V> tree) {
        ...
        if (children.size() > tree.getOrder()) {
            ...
            for (int i = 0; i < rightSize - 1; i++) {
                ...
                right.entries_val.add(entries_val.get(leftSize + i));
            }
            ...
            if (parent != null) {
                ...
                parent.entries_val.add(index, entries_val.get(leftSize - 1));
                entries = null;
                entries_val = null;
                children = null;
                ...
                parent.entries_val.add(entries_val.get(leftSize - 1));
                entries = null;
                entries_val = null;
                children = null;
            }
    ...
    protected void updateRemove(BplusTree<K, V> tree) {
        ...
        if (children.size() < tree.getOrder() / 2 || children.size() < 2) {
            ...
            if (previous != null
                ...
                entries_val.add(0, parent.entries_val.get(preIndex));
                parent.entries.set(preIndex, previous.entries.remove(idx - 1));
                parent.entries_val.set(preIndex, previous.entries_val.remove(idx - 1));
                return;
            }
            ...
            if (next != null
                ...
                entries_val.add(parent.entries_val.get(preIndex));
                parent.entries.set(preIndex, next.entries.remove(0));
                parent.entries_val.set(preIndex, next.entries_val.remove(0));
                return;
            }
            ...
            if (previous != null
                ...
                previous.entries_val.add(parent.entries_val.get(indexPre));
                for (int i = 0; i < entries.size(); i++) {
                    previous.entries.add(entries.get(i));
                    previous.entries_val.add(entries_val.get(i));
                }
                children = previous.children;
                entries = previous.entries;
                entries_val = previous.entries_val;
                //更新父节点的关键字列表
                parent.children.remove(previous);
                previous.parent = null;
                previous.children = null;
                previous.entries = null;
                previous.entries_val = null;
                parent.entries.remove(parent.children.indexOf(this));
                parent.entries_val.remove(parent.children.indexOf(this));
                if ((!parent.isRoot
            ...
            if (next != null
                ...
                entries_val.add(parent.entries_val.get(index));
                for (int i = 0; i < next.entries.size(); i++) {
                    entries.add(next.entries.get(i));
                    entries_val.add(next.entries_val.get(i));
                }
                parent.children.remove(next);
                next.parent = null;
                next.children = null;
                next.entries = null;
                next.entries_val = null;
                parent.entries.remove(parent.children.indexOf(this));
                parent.entries_val.remove(parent.children.indexOf(this));
    ...
    public Set<V> remove(K key, BplusTree<K, V> tree) {
        ...
        if (isLeaf) {
            ...
            if (previous != null &&
                ...
                entries_val.add(0, previous.entries_val.remove(size - 1));
                int index = parent.children.indexOf(previous);
                parent.entries.set(index, entries.get(0));
                parent.entries_val.set(index, entries_val.get(0));
                return remove(key);
            }
            ...
            if (next != null
                ...
                entries_val.add(next.entries_val.remove(0));
                int index = parent.children.indexOf(this);
                parent.entries.set(index, next.entries.get(0));
                parent.entries_val.set(index, next.entries_val.get(0));
                return remove(key);
            }
